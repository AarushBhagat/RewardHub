  ...
  return (
    <div className="space-y-8">
      <div className="text-center max-w-2xl mx-auto">
        ...
        <p className="text-slate-500 dark:text-slate-400">See how you rank among your peers. Top 3 employees each quarter receive a special bonus!</p>
      </div>
      ...
      <div className="glass-card max-w-3xl mx-auto p-0 overflow-hidden">
        <div className="divide-y divide-slate-100 dark:divide-slate-700/50">
          {rest.map((emp, i) => {
            const rank = i + 4;
            // Highlight row if it's the current user (EMP001)
            const isMe = emp.id === 'EMP001';
            ...
            return (
              <motion.div 
                ...
                key={emp.id} 
                className={`flex items-center justify-between p-4 ${isMe ? 'bg-primary/5 border-l-4 border-l-primary' : 'hover:bg-slate-50 dark:hover:bg-slate-800/30'} transition-colors`}
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                      {emp.name} {isMe && <span className="text-xs bg-primary text-white px-2 py-0.5 rounded-full">You</span>}
                    </p>
